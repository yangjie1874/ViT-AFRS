...
def total_loss(inputs, targets, reduction="mean", bg_weight=0.2, num_classes=None, tversky_weight=0.3, alpha=0.5, beta=0.5, epsilon=1e-7):
    probs = F.softmax(inputs,dim=1)
    one_hot = F.one_hot(targets, num_classes=num_classes+1).float()
    intersection = torch.sum(probs*one_hot,dim=0)
    false_positive = torch.sum(probs*(1-one_hot),dim=0)
    false_negative = torch.sum((1-probs)*one_hot,dim=0)
    tversky_index = (intersection+epsilon)/(intersection+alpha*false_positive+beta*false_negative+epsilon)
    tversky_loss = 1-tversky_index

    weight = torch.ones_like(tversky_loss)
    weight[num_classes] = bg_weight
    tversky_loss = tversky_loss *weight

    if reduction == 'mean':
        tversky_loss = tversky_loss.mean()
    total_loss1 = tversky_weight*tversky_loss
    return  total_loss1
...
class AdaptiveFeatureProcessor(nn.Module):
    """
    将AdditiveTokenMixer的核心思想应用到特征处理中
    处理输入特征并生成注意力权重
    """
    ...
    def forward(self, x):
        """
        输入: x [batch_size, num_samples, dim]
        输出: 处理后的特征和注意力权重
        """
        ...
        q, k, v = qkv.chunk(3, dim=-1)  # 每个都是 [B, N, dim]

        # 应用操作获得注意力权重
        q_weights = self.oper_q(q)  # [B, N, 1]
        k_weights = self.oper_k(k)  # [B, N, 1]

        # 融合Q和K的权重信息
        combined_weights = q_weights * k_weights  # [B, N, 1]

        # 深度处理
        qk_combined = self.dwc(q + k)  # [B, N, dim]

        # 加权特征
        weighted_features = qk_combined * v * combined_weights  # [B, N, dim]

        # 最终投影
        output = self.proj(weighted_features)
        output = self.proj_drop(output)

        return output, combined_weights.squeeze(-1)  # [B, N, dim], [B, N]


class ProtoCenterAttention_Enhanced(nn.Module):
    """
    整合AdditiveTokenMixer思想的增强版ProtoCenterAttention
    """

    def __init__(self, input_size, use_adaptive_processor=True, proj_drop=0.):
        super(ProtoCenterAttention_Enhanced, self).__init__()
        self.input_size = input_size
        self.use_adaptive_processor = use_adaptive_processor

        if use_adaptive_processor:
            # 使用增强的特征处理器
            self.feature_processor = AdaptiveFeatureProcessor(
                input_size, proj_drop=proj_drop
            )
        else:
            # 回退到简单版本
            self.attention_weights = nn.Sequential(
    ...
    def forward(self, x):
        """
        输入: x [batch_size, num_samples, input_size]
        输出: (原型特征, 注意力分数)
        """
        if self.use_adaptive_processor:
            # 使用增强的处理流程
            processed_features, raw_attention = self.feature_processor(x)

            # 归一化注意力权重
            attention_scores = F.softmax(raw_attention, dim=1).unsqueeze(-1)  # [B, N, 1]

            # 加权聚合
            weighted_features = processed_features * attention_scores
            aggregated_features = weighted_features.sum(dim=1)  # [B, dim]

        else:
            # 简单版本的处理流程
            attention_scores = self.attention_weights(x)  # [B, N, 1]
            weighted_features = x * attention_scores
            aggregated_features = weighted_features.sum(dim=1)  # [B, dim]
        ...
        return output, attention_scores.squeeze(-1)  # [B, dim], [B, N]


class InstReweight(nn.Module):
    """
    使用增强版ProtoCenterAttention的InstReweight
    """

    def __init__(self, c_in, ratio=0.5, use_enhanced_attention=True, proj_drop=0.):
        super(InstReweight, self).__init__()
        self.proto_center_attn = ProtoCenterAttention_Enhanced(
            c_in,
            use_adaptive_processor=use_enhanced_attention,
            proj_drop=proj_drop
        ).cuda()
        self.ratio = ratio
    ...
    def forward(self, x):
        ...
        out = (self.ratio * F.normalize(x_attn, dim=-1) +
               (1 - self.ratio) * F.normalize(x.mean(dim=1), dim=-1))
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def mask_forward(self, features, boxes, class_labels, class_weights, gt_masks=None, feature_dict=None):
        ...
        if self.set_cd_vito:
            bg_roi_feats = roi_feats.transpose(-2, -1) @ self.bg_tokens.T
        ...
        if self.set_cd_vito:
            fg_roi_feats = roi_feats.transpose(-2, -1) @ class_weights.T
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.training:
            ...
            if not self.only_train_mask:
                ...
                if isinstance(logits, list):
                    ...
                    for i, l in enumerate(logits):
                        loss1 = focal_loss(l, class_labels, num_classes=num_active_classes, bg_weight=self.bg_cls_weight)
                        loss2 =loss1+total_loss(l, class_labels, num_classes=num_active_classes, bg_weight=self.bg_cls_weight)
                        loss_dict[f'focal_loss_{i}'] = loss2
                else:
                    _log_classification_stats(logits.detach(), class_labels)
                    loss1 = focal_loss(logits, class_labels, num_classes=num_active_classes,
                                      bg_weight=self.bg_cls_weight)
                    loss2 =loss1+total_loss(logits, class_labels, num_classes=num_active_classes,
                                                         bg_weight=self.bg_cls_weight)
                    loss_dict['focal_loss'] = loss2
            ...
            if self.set_cd_vito and self.num_train_classes != 1:
                ...
                # 遥感背景原型定义 - 模拟复杂遥感背景
                bg_prototypes = F.normalize(self.bg_tokens.weight, dim=-1)
                ...
                for i in range(len(class_prototypes)):
                    ...
                    # 背景感知负样本：其他类的原型 + 背景干扰 + 同域提示
                    # 模拟遥感图像中目标与复杂背景混合的真实场景
                    # 添加遥感背景复杂性：随机选择背景类型并融入负样本
                    bg_rand_idx = torch.randint(0, len(bg_prototypes), (len(negative_samples),))
                    bg_interference = bg_prototypes[bg_rand_idx]
                    #背景干扰强度控制：0.15保证不完全掩盖类别特征，但增加识别难度
                    negative_samples_with_bg = negative_samples + 0.15 * bg_interference
                    negative_keys.append(negative_samples_with_bg)
